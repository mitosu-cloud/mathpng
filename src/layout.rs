...
impl<'a> Renderer<'a> {
    ...
    fn layout_scripts(
        ...
        if let Some(sup_expr) = sup {
            ...
            children.push(PositionedNode { x: sup_x, y: shift_up, node: sup_node.clone() });
        ...
        if let Some(sub_expr) = sub {
            ...
            children.push(PositionedNode { x: sub_x, y: -shift_down, node: sub_node.clone() });
    ...
    fn layout_radical(
        ...
        LayoutNode {
            ...
            content: LayoutContent::HBox {
                children: vec![
                    PositionedNode { x: 0.0, y: 0.0, node: surd_glyph },
                    PositionedNode {
                        x: surd_width,
                        y: overbar_y,
                        node: LayoutNode {
                    ...
                    PositionedNode { x: surd_width, y: 0.0, node: radicand_node },
                ],
            },
        }
    }
    ...
    fn layout_delimited(
        ...
        if let Some(open_char) = open {
            ...
            children.push(PositionedNode { x, y: 0.0, node: delim.clone() });
            x += delim.width;
        }
        ...
        let max_ascent = children.iter().map(|c| c.y + c.node.ascent).fold(0.0_f32, f32::max);
        let max_descent = children.iter().map(|c| (-c.y) + c.node.descent).fold(0.0_f32, f32::max);
    ...
    fn layout_big_operator(
        ...
        if display_mode && limits {
            ...
            children.push(PositionedNode { x: op_x, y: 0.0, node: op_node.clone() });
            ...
            if let Some(a) = above_node {
                ...
                children.push(PositionedNode { x: above_x, y: above_y, node: a });
            }
            ...
            if let Some(b) = below_node {
                ...
                children.push(PositionedNode { x: below_x, y: below_y, node: b });
            }
    ...
    fn layout_text(&self, text: &str, font_size_px: f32) -> LayoutNode {
        let exprs: Vec<MathExpr> = text
            .chars()
            .map(|c| MathExpr::Glyph { codepoint: c, variant: MathVariant::Normal })
            .collect();
    ...
    fn layout_accent(
        ...
        LayoutNode {
            ...
            content: LayoutContent::VBox {
                children: vec![
                    PositionedNode { x: accent_x.max(0.0), y: accent_y, node: accent_node },
                    PositionedNode { x: 0.0, y: 0.0, node: base_node },
                ],
            },
        }
    }
